...
const startTimer = () => {
    const storageKey = `fapshi_timer_${props.transaction.reference}`;
...
const isOrange = computed(() => props.transaction.payment_method === 'orange');
const amount = computed(() => props.transaction.amount);

// Intermediate validation screen state
const isConfirmed = ref(false);
const confirmedPhone = ref('');
const selectedOperator = ref<'mtn' | 'orange' | ''>('');

// Auto-detect prefix as they type
watch(confirmedPhone, (newVal) => {
    const clean = newVal.replace(/\D/g, '');
    let localNumber = clean;
    if (localNumber.startsWith('237')) {
        localNumber = localNumber.substring(3);
    }
    
    if (/^(69|655|656|657|658|659)/.test(localNumber)) {
        selectedOperator.value = 'orange';
    } else if (/^(67|68|650|651|652|653|654)/.test(localNumber)) {
        selectedOperator.value = 'mtn';
    } else {
...
const isPhoneValid = computed(() => {
    ...
    const cleanOriginal = props.transaction.payment_phone.replace(/\D/g, '');
...
const checkStatus = async () => {
    ...
    try {
        const response = await fetch(`/fapshi/check-status/${props.transaction.reference}`, {
            method: 'POST',
            headers: {
        ...
        if (response.ok) {
            ...
            statusMessage.value = data.message || "Erreur de vérification.";
            statusType.value = 'error';
        }
    } catch (e) {
        statusMessage.value = "Erreur réseau. Veuillez réessayer.";
        statusType.value = 'error';
    } finally {
        ...
        <div class="flex-1 max-w-md w-full mx-auto p-4 flex flex-col gap-5">
            ...
            <template v-if="!isConfirmed">
                ...
                <button
                    ...
                    :class="isPhoneValid ? 'bg-[#6366f1] hover:bg-[#4f46e5]' : 'bg-gray-400 opacity-60 cursor-not-allowed'"
                >
                    {{ t('btnConfirmSubmit') }}
                </button>
            </template>
                ...
                <div class="bg-white rounded-[28px] p-6 shadow-sm border border-gray-100 flex flex-col gap-4">
                    ...
                    <div class="bg-gray-50 border border-gray-100 rounded-2xl p-4 flex items-center justify-between mt-1">
                        <div class="flex flex-col">
                            ...
                            <span class="text-lg font-bold font-mono tracking-wide text-gray-800 mt-1">{{ transaction.payment_phone }}</span>
                        </div>

                        <button @click="copyText(transaction.payment_phone, 'phone')" class="flex items-center gap-1.5 text-xs font-semibold text-gray-600 bg-white border border-gray-200 shadow-sm px-3.5 py-1.5 rounded-xl hover:bg-gray-50 transition-colors">
